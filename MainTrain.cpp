...
int main(){
    // TODO:
    // Code work fine, but sometimes ( rarely) it is hanging out with client 2.
    // from what i can tell, it happens when it alert (timeout), it kill the thread, but then server.stop() is stuck ( can't join killed thread)
    // Not sure I can fix it. It is also quite hard to debug because of timing.
